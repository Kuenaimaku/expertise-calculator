<template>
  ...
  >
    <template slot="content">
      <p>
        <span class="option-title">Magic Control</span>: minimum Class 3 Rank 0.
        <span class="option-title">40%</span>
        is converted to Chain Expertise ({{parseExpertise(expertise.magicControl.value * 0.4)}})
      </p>
      <p>
...
export default {
  ...
  computed: {
    ...
    classRank() {
      ...
      var a = Number.parseInt(Math.min((magicControl + crushingTechnique + demonology),8800)) / 100;
      var b = a.toString();
      if (a === 0) {
        return "Class 0 Rank 0";
      } else if (a >= 10) {
